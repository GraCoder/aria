...
if (typeof browser !== "undefined") {
	...
	isChrome = true;
	if (navigator.userAgent.includes("Vivaldi/")) isVivaldi = true; // Vivaldi is a subbrowser of chrome :|
}
...
function getDomain(url) {
	var TLDs = [
		...
		"ax", "az", "ba", "bb", "bd", "be", "bf", "bg", "bh", "bi", "biz", "bj", "bm", "bn", "bo", "br", "bs", "bt", "bv",
		"bw", "by", "bz", "ca", "cat", "cc", "cd", "cf", "cg", "ch", "ci", "ck", "cl", "cm", "cn", "co", "com", "coop", "cr", "cu", "cv", "cx",
		"cy", "cz", "de", "dj", "dk", "dm", "do", "dz", "ec", "edu", "ee", "eg", "er", "es", "et", "eu", "fi", "fj", "fk", "fm", "fo", "fr",
		"ga", "gb", "gd", "ge", "gf", "gg", "gh", "gi", "gl", "gm", "gn", "gov", "gp", "gq", "gr", "gs", "gt", "gu", "gw", "gy", "hk", "hm", "hn", "hr", "ht", "hu",
		"id", "ie", "il", "im", "in", "info", "int", "io", "iq", "ir", "is", "it", "je", "jm", "jo", "jobs", "jp", "ke", "kg", "kh", "ki", "km", "kn", "kp",
		"kr", "kw", "ky", "kz", "la", "lb", "lc", "li", "lk", "lr", "ls", "lt", "lu", "lv", "ly", "ma", "mc", "md", "me", "mg", "mh", "mil", "mk",
		"ml", "mm", "mn", "mo", "mobi", "mp", "mq", "mr", "ms", "mt", "mu", "museum", "mv", "mw", "mx", "my", "mz", "na", "name", "nc", "ne", "net", "nf",
		"ng", "ni", "nl", "no", "np", "nr", "nu", "nz", "om", "org", "pa", "pe", "pf", "pg", "ph", "pk", "pl", "pm", "pn", "pr", "pro", "ps", "pt", "pw",
		"py", "qa", "re", "ro", "rs", "ru", "rw", "sa", "sb", "sc", "sd", "se", "sg", "sh", "si", "sj", "sk", "sl", "sm", "sn", "so", "sr", "st", "su", "sv",
		"sy", "sz", "tc", "td", "tel", "tf", "tg", "th", "tj", "tk", "tl", "tm", "tn", "to", "tp", "tr", "travel", "tt", "tv", "tw", "tz", "ua", "ug", "uk", "us",
		"uy", "uz", "va", "vc", "ve", "vg", "vi", "vn", "vu", "wf", "ws", "xn--0zwm56d", "xn--11b5bs3a9aj6g", "xn--3e0b707e", "xn--45brj9c", "xn--80akhbyknj4f",
		"xn--90a3ac", "xn--9t4b11yi5a", "xn--clchc0ea0b2g2a9gcd", "xn--deba0ad", "xn--fiqs8s", "xn--fiqz9s", "xn--fpcrj9c3d", "xn--fzc2c9e2c", "xn--g6w251d",
		"xn--gecrj9c", "xn--h2brj9c", "xn--hgbk6aj7f53bba", "xn--hlcj6aya9esc7a", "xn--j6w193g", "xn--jxalpdlp", "xn--kgbechtv", "xn--kprw13d", "xn--kpry57d",
		"xn--lgbbat1ad8j", "xn--mgbaam7a8h", "xn--mgbayh7gpa", "xn--mgbbh1a71e", "xn--mgbc0a9azcg", "xn--mgberp4a5d4ar", "xn--o3cw4h", "xn--ogbpf8fl", "xn--p1ai",
		"xn--pgbs0dh", "xn--s9brj9c", "xn--wgbh1c", "xn--wgbl6a", "xn--xkc2al3hye2a", "xn--xkc2dl3a5ee0h", "xn--yfro4i67o", "xn--ygbi2ammx", "xn--zckzah",
		"xxx", "ye", "yt", "za", "zm", "zw",
	].join();
	...
	if (typeof tid != "undefined") actual_domain = actual_domain + tid;
	else actual_domain = actual_domain + ".com";
...
function getCookies(url, callback) {
	...
	let blacklistDecode = ["mycdn.me"];

	if (isChrome) {
		BrowserNameSpace.cookies.getAll(urlQuery, (urlcookies) => {
			let cookieArray = [];
			if (blacklistDecode.indexOf(domain) == -1)
				cookieArray = urlcookies.map(
					(cookie) => denCode(cookie.name) + "=" + denCode(cookie.value)
				);
			else
				cookieArray = urlcookies.map(
		...
		BrowserNameSpace.cookies.getAll(urlQuery).then((urlcookies) => {
			let cookieArray = [];
			if (blacklistDecode.indexOf(domain) == -1)
				cookieArray = urlcookies.map((cookie) => {
					return denCode(cookie.name) + "=" + denCode(cookie.value);
				});
			else
				cookieArray = urlcookies.map((cookie) => {
...
BrowserNameSpace.runtime.onMessage.addListener(function (
	request,
	sender,
	sendResponse
) {
...
if (isChrome && !isVivaldi) {
	...
	BrowserNameSpace.downloads.onDeterminingFilename.addListener(
		(downloadItem, suggest) => {
			if (!interruptDownloads) {
				// pdm-chrome-wrapper not reachable
				suggest();
				return;
			}
			...
			if (!url || isBlackListed(url) || (0 < downloadItem.fileSize && downloadItem.fileSize < MIN_FILE_SIZE_INTERRUPT)) {
				suggest();
			} else {
				...
				msg.mime = downloadItem["mime"];
				msg.fileSize = downloadItem["fileSize"];
...
BrowserNameSpace.downloads.onCreated.addListener(function (downloadItem) {
	...
	if (isFF || isVivaldi) {
		...
		if (
			...
			(0 < downloadItem.fileSize &&
				downloadItem.fileSize < MIN_FILE_SIZE_INTERRUPT) // File size is determined and is less than MIN_FILE_SIZE
		) {
			return;
		} else {
			setTimeout(() => {
...
function isBlackListed(url) {
	...
	if (url.startsWith("blob://"))
		// TODO: Persepolis currently can't handle blob type
		return true;
...
async function setInterruptDownload(interrupt) {
	...
	if (interrupt) {
		...
		BrowserNameSpace.browserAction.setIcon({
			path: "./icons/icon_disabled_32.png",
		});
	}
}
...
async function ConfigGetVal(key, default_value = "") {
	...
	if (["true", "false"].includes(configValue)) return configValue == "true"; // Converts string Boolean to Boolean
	return configValue;
}
...
function setContextMenu(newState) {
	if (!newState) {
		...
		try {
			...
			BrowserNameSpace.contextMenus.create(
				{
					title: "Download Selected links with Aria",
			...
			BrowserNameSpace.contextMenus.create(
				{
					title: "Download All Links with Aria",
...
BrowserNameSpace.contextMenus.onClicked.addListener(function (info, tab) {
	...
	switch (info.menuItemId) {
		...
		case "download_links_with_pdm":
			BrowserNameSpace.tabs.executeScript(
				null,
				{ file: "/scripts/injector.js" },
				() => {
					BrowserNameSpace.tabs.executeScript(null, {
						file: "/scripts/getselected.js",
					});
				}
			);
			break;
